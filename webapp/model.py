#Эта модель на будущее, которую я пока не могу использовать, потому что она слишком сложная
from flask_sqlalchemy import SQLAlchemy

db = SQLAlchemy()

editions = db.Table('editions',
    db.Column('edition_id', db.ForeignKey('edition.id'), primary_key=True),
...
class Edition(db.Model):
    ...
    authors = db.relationship('Author', secondary=editions, backref=db.backref('authors'))
    ...
    other_editions = db.relationship('Edition', secondary=other_editions, backref=db.backref('edition'))
...
class Author(db.Model):
    ...
    editions = db.relationship('Edition', secondary=editions, backref=db.backref('editions'))
